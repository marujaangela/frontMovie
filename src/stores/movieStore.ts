...
export const useMovieStore = defineStore('movies', () => {
  ...
  const setSelectedGenre = ref<number | null>(null);
...
// Computed Property: Gruppiert Filme nach Genre
  const moviesByGenre = computed(() => {
    ...
    genres.value.forEach((genre) => {
      grouped[genre.name] = currentMovies.value.filter(
        (movie) => movie.genre === genre.id // Direkter Vergleich zwischen numerischen Werten
      );
    });
    return grouped;
  });
  ...
  const filteredMovies = computed(() => {
    ...
    // Filter nach Genre (numerische ID)
    if (setSelectedGenre.value) {
      filtered = filtered.filter(
        (movie) => movie.genre === setSelectedGenre.value // Numerischer Vergleich
      );
    }
    ...
    if (searchQuery.value) {
      ...
      filtered = filtered.filter((movie) => {
        return (
          movie.title.toLowerCase().includes(query) || // Suche im Titel
          genres.value.some((genre) => {
            return (
              genre.id === movie.genre && // Numerischer Vergleich zwischen Genre-ID und Movie-Genre
              genre.name.toLowerCase().includes(query) // Suche im Genre-Namen
            );
          })
        );
      });
  ...
  async function removeGenre(id: number): Promise<void> {
    try {
      await axios.delete(`${apiEndpointTwo}/${Number(id)}`);
...
// **Methode: Ansicht ändern**
  ...
  async function toggleWatched(id: number): Promise<void> {
  ...
  function generateRecommendation(genreId: UnwrapRef<Genre['id']>):void  {
    const eligibleMovies = currentMovies.value.filter(
      (movie) => movie.genre == genreId && !movie.watched
    );
